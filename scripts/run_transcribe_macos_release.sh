...
chunk_ms="${PARAKEET_STREAM_CHUNK_MS:-160}"
hop_ms="${PARAKEET_STREAM_HOP_MS:-80}"
...
decode_only_when_speech="${PARAKEET_DECODE_ONLY_WHEN_SPEECH:-1}"
...
max_batch_sec="${PARAKEET_STREAM_MAX_BATCH_SEC:-0.25}"
max_buffer_sec="${PARAKEET_STREAM_MAX_BUFFER_SEC:-4}"
backlog_soft_sec="${PARAKEET_STREAM_BACKLOG_SOFT_SEC:-1.5}"
backlog_target_sec="${PARAKEET_STREAM_BACKLOG_TARGET_SEC:-0.4}"
latest_first="${PARAKEET_STREAM_LATEST_FIRST:-1}"
max_symbols_per_step="${PARAKEET_TDT_MAX_SYMBOLS_PER_STEP:-4}"
max_tokens_per_chunk="${PARAKEET_TDT_MAX_TOKENS_PER_CHUNK:-64}"
