...
private func defaultImportedParakeetModelDirectory() -> URL? {
...
private func parakeetModelDirectoryCandidates() -> [URL] {
    ...
    if let imported = defaultImportedParakeetModelDirectory() {
        candidates.append(imported)
    }

    if let resourceRoot = Bundle.main.resourceURL {
        candidates.append(resourceRoot.appendingPathComponent("parakeet-tdt-0.6b-v2", isDirectory: true))
        candidates.append(resourceRoot.appendingPathComponent("Resources/parakeet-tdt-0.6b-v2", isDirectory: true))
        candidates.append(resourceRoot.appendingPathComponent("Models/parakeet-tdt-0.6b-v2", isDirectory: true))
    }
    ...
    let repoRoot = sourceURL
        .deletingLastPathComponent() // .../Sources/TranscribeAppleAppSupport
...
public struct TranscribeAppleRootView: View {
    public init() {}
...
struct ContentView: View {
    ...
    #if os(iOS)
    private let metricsPanelHeight: CGFloat = 74
    #else
    private let metricsPanelHeight: CGFloat = 68
    #endif
    ...
    var body: some View {
        ...
        .fileImporter(
            ...
            case .success(let urls):
                ...
                vm.importModelDirectory(from: url)
            case .failure(let error):
                vm.noteModelImportFailure(error)
            }
        }
        #endif
        #else
    ...
    private var content: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            .foregroundStyle(.secondary)
            .frame(maxWidth: .infinity, minHeight: metricsPanelHeight, maxHeight: metricsPanelHeight, alignment: .topLeading)
            .accessibilityElement(children: .combine)
            .accessibilityLabel("Metrics")
            .accessibilityValue(vm.metrics)
...
final class MicTranscriptionViewModel: ObservableObject, @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "transcribe.apple.inference")
    private let startupQueue = DispatchQueue(label: "transcribe.apple.startup", qos: .userInitiated)
    ...
    private let audioTapBufferFrames: AVAudioFrameCount = {
        if let raw = ProcessInfo.processInfo.environment["PARAKEET_AUDIO_TAP_BUFFER_FRAMES"],
           let parsed = Int(raw),
           parsed > 0 {
            return AVAudioFrameCount(parsed)
        }
        #if os(iOS)
        return 1024
        #else
    ...
    private let preferredIOBufferDurationSec: Double = {
        if let raw = ProcessInfo.processInfo.environment["PARAKEET_AUDIO_IO_BUFFER_MS"],
    ...
    func noteModelImportFailure(_ error: Error) {
        DispatchQueue.main.async {
            self.status = "Model import failed: \(error.localizedDescription)"
        }
    }

    func importModelDirectory(from externalURL: URL) {
        startupQueue.async { [weak self] in
            ...
            do {
                ...
                guard let destination = defaultImportedParakeetModelDirectory() else {
                    throw NSError(
                        domain: "transcribe-apple",
                        code: 30,
                        userInfo: [NSLocalizedDescriptionKey: "No writable import directory is available on this device."]
                    )
                }
                ...
                let resolvedEncoderSuffix = (self.encoderModelSuffix?.isEmpty == false) ? self.encoderModelSuffix! : self.modelSuffix
                let resolvedDecoderSuffix = preferredParakeetDecoderSuffix(
                    at: destination,
                    suffix: self.modelSuffix,
                    decoderSuffix: self.decoderModelSuffix
                )
                guard hasParakeetArtifacts(
                    at: destination,
                    suffix: self.modelSuffix,
                    encoderSuffix: self.encoderModelSuffix,
                    decoderSuffix: self.decoderModelSuffix
                ) else {
                    throw NSError(
                        domain: "transcribe-apple",
                        ...
                        userInfo: [
                            NSLocalizedDescriptionKey:
                                "Imported folder is missing encoder '\(resolvedEncoderSuffix)', decoder '\(resolvedDecoderSuffix)', or vocab.txt."
                        ]
                    )
                }

                DispatchQueue.main.async {
                    self.modelDirectory = destination.path
                    self.status = "Imported model folder. Tap Start to load."
                }
            } catch {
                self.noteModelImportFailure(error)
            }
        }
    }
    ...
    private func start() {
        ...
        guard let dirURL = resolveParakeetModelDirectory(
            ...
            #if os(iOS)
            status = "Model artifacts not found. Import or bundle parakeet-tdt-0.6b-v2 for encoder '\(resolvedEncoderSuffix)' and decoder '\(resolvedDecoderSuffix)'."
            #else
    ...
    private func configureAudioSessionIfNeeded() throws {
        ...
        try session.setPreferredIOBufferDuration(preferredIOBufferDurationSec)
    ...
    private func configureAndStartAudio(engine: InferenceEngine) {
        ...
        do {
            ...
            func configureInputPath(voiceProcessing: Bool) throws -> AVAudioFormat {
                ...
                inputNode.installTap(onBus: 0, bufferSize: audioTapBufferFrames, format: inputFormat) { [weak self] buffer, _ in
